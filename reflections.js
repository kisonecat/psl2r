...
function draw() {
  ...
  ctx.beginPath();
  ctx.fillStyle = "#888";
  ctx.arc(initialPoint.x, initialPoint.y, 5*pixels, 0, 2*Math.PI);
  ctx.fill();

  let p = initialPoint;

  for( var i = 0; i < 3; i ++ ) {
    let q = applyMatrix( involution(points[i]), p );
    ctx.beginPath();
    ...
    ctx.beginPath();
    ctx.fillStyle = "#888";
    ctx.arc(q.x, q.y, 5*pixels, 0, 2*Math.PI);
    ctx.fill();
